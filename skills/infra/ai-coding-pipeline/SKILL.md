...
Quick Flow 5 阶段（4 个 Kanban task）：
...
1. **Intake**（orchestrator）：需求澄清，判定走 Quick Flow（见下方决策树）。解析 `PIPELINE_WORKSPACE`，固定目标项目，固化目标项目约定。
2. **T1 RFC/SPEC/Design（yquantprincipal）**：一个 Kanban task 同时产出 RFC / SPEC / Design 三份独立文档，分别落到 `docs/{rfc,spec,design}/{module}/` 对应路径。三份文档保留三层职责边界，**不得合并章节**。T1 body 必须在顶部显式声明"本任务走 Quick Flow"。
3. **T2 Implement（yquantdeveloper）**：基于已确认的 SPEC + Design 做最小范围实现。T3 Verify 不得跳过。
4. **T3 Verify（yquanttester）**：独立验证 + 端到端 smoke test，**必须做数据合理性抽样**（P-11）。
5. **T4 Closeout（orchestrator）**：执行 DESIGN-10-004 §3.5 自审清单（≥11 项，实际 13 项），逐项 ✅/❌ 标注。**无独立 Reviewer**，自审替代。
...
P-1~P-11 pitfalls **在 Quick Flow 中全部继续适用**（DESIGN-10-004 §3.7 兼容性矩阵），无例外。

## Quick Flow 阶段门禁

| 阶段 | 门禁 | 验证方式 |
|---|---|---|
| T1 RFC/SPEC/Design | 三份文档存在 + 引用关系正确 + 章节结构完整 | orchestrator 校验 |
| T2 Implement | 代码编译/语法通过 + 单元测试通过 | developer 自检 + tester 验证 |
| T3 Verify | 验收标准全部通过 + 端到端 smoke test 通过 + 数据合理性抽样 | tester 报告 |
| T4 Closeout | DESIGN-10-004 §3.5 自审清单 13 项全部 ✅ | orchestrator 逐项核查 |

T4 自审清单 13 项详见 `references/pipeline.md` 的 Quick Flow 章节。

### Quick Flow 文档交叉引用

- RFC：`docs/rfc/10_infra/RFC-10-004-yquant-ai-coding-pipeline-skill-sync.md` §12（Quick Flow 扩展，背景/动机/触发条件/Kanban 链/Closeout 自审清单 13 项）
- SPEC：`docs/spec/10_infra/SPEC-10-004-yquant-ai-coding-pipeline-skill-sync.md` §13（Quick Flow 可执行契约，13.1-13.9）
- Design：`docs/design/10_infra/DESIGN-10-004-yquant-ai-coding-pipeline-skill-sync.md`（§3.1 时序图、§3.2-§3.5 task body 模板、§3.6 orchestrator 改动点、§3.7 P-1~P-11 兼容矩阵、§3.8 降级/升级路径）
...
## Quick Flow 5 阶段流程模式（2026-06-29 新增）

> 完整 RFC/SPEC/Design 文档：RFC-10-004 §12 / SPEC-10-004 §13 / DESIGN-10-004（yquant 项目 `docs/` 下）
> 
> 本次用 Quick Flow 改造 Quick Flow 自身的实战案例：`t_e723d3cb` (T1) → `t_e20d7bd9` (T2) → `t_5ac8129d` (T3) → T4 Closeout

**5 阶段 4 task Kanban 链**：
```
...
- **Quick Flow (5 阶段 / 4 task)**：中风险 → 去掉 Review，以 **Closeout 自审清单 (13 项)** 替代
...
**P-1~P-11 pitfalls 全部适用于 Quick Flow**（不变）。
